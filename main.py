...
class ConfigurationManager:
    ...
    async def start_redis_with_config(self) -> bool:
        """Start Redis with configuration from redis.conf."""
        ...
        # Start Redis with config if available
        ...
        try:
            ...
            if result.returncode == 0 and 'PONG' in result.stdout:
                ...
                logger.error("❌ Redis failed to respond")
                return False
        except Exception as e:
...
async def initialize_revolutionary_system():
    ...
    try:
        ...
        # Step 1: Start Redis with configuration
        logger.info("📊 Step 1: Starting Redis with configuration...")
        redis_success = await config_manager.start_redis_with_config()
        if not redis_success:
            logger.warning("⚠️ Redis startup had issues, continuing with fallback")
        ...
        # Try external Qdrant first, fallback to in-memory
        if qdrant_success:
        ...
        logger.info(f"📊 Redis: {'✅' if redis_success else '⚠️'}")
